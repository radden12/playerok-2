...
class Core:
    ...
    def _prepare_playerok_cert(self, account_module: Any) -> None:
        module_file = Path(str(getattr(account_module, "__file__", "") or ""))
        package_cert = module_file.with_name("cacert.pem") if module_file.name else Path()
        if module_file.name and package_cert.is_file():
            return

        try:
            from certifi import where as certifi_cert_path

            source = Path(certifi_cert_path())
        except ImportError:
            import ssl

            source = Path(str(ssl.get_default_verify_paths().cafile or ""))
        if not source.is_file():
            raise FileNotFoundError("не найден системный пакет корневых сертификатов")
        runtime_dir = self.data_dir / "runtime" / "playerokapi"
        runtime_dir.mkdir(parents=True, exist_ok=True)
        target = runtime_dir / "cacert.pem"
        if not target.is_file() or target.stat().st_size != source.stat().st_size:
            shutil.copyfile(source, target)
        # PlayerokAPI строит путь к cacert.pem относительно глобального __file__
        # своего модуля. В опубликованном wheel этот файл пропущен, поэтому
        # направляем библиотеку на безопасную копию внутри /app/data.
        account_module.__file__ = str(runtime_dir / "account.py")

    def _new_playerok_account(self, cookies: str, user_agent: str) -> Any:
        import playerokapi.account as account_module

        self._prepare_playerok_cert(account_module)
